...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  create_deal: { label: "Create Opportunity", icon: Briefcase, border: "border-l-primary" },
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  ...
  { value: "new_contact_created", label: "New Lead Created", hint: "When a lead is auto-created from an incoming message" },
...
function TriggerCard({
  ...
  return (
    ...
    <div className="z-10 w-full max-w-[320px] sm:w-80">
      <div className="rounded-lg border border-slate-800 border-l-4 border-l-blue-500 bg-slate-900 shadow-sm">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
  ...
  return (
    <>
      <div className={cn("z-10 flex flex-col", width)}>
        <div
          className={cn(
            "rounded-lg border border-slate-800 border-l-4 bg-slate-900 shadow-sm",
            meta.border,
          )}
        >
          <button
